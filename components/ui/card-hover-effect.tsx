      ...
      {items.map((item, idx) => (
        <Link
          ...
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 block h-full w-full rounded-3xl bg-neutral-400 dark:bg-primary/[0.5]"
...
export const CardDescription = ({
  ...
  return (
    <p className={cn("mt-8 text-sm leading-relaxed tracking-wide text-zinc-400", className)}>{children}</p>
  );
};
