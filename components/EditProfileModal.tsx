...
export default function EditProfileModal({ profileData, userId }: EditProfileModalProps) {
  ...
  async function onSubmit(profileData: z.infer<typeof profileSchema>) {
    ...
    try {
      ...
      const links = [profileData.link1, profileData.link2, profileData.link3, profileData.link4].filter(
        (link): link is string => link != ""
      ); // check if link is string and not empty
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        {userId === profileData.user_id ? <Button variant="outline">Edit Profile</Button> : null}
      </DialogTrigger>
        ...
        <Form {...profileForm}>
          <form onSubmit={profileForm.handleSubmit(onSubmit)} className="space-y-4">
            <div className="flex items-center justify-center gap-5 p-5">
              ...
              <label
                htmlFor="user-file"
                className="user-edit inline-flex h-10 cursor-pointer items-center justify-center whitespace-nowrap rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground ring-offset-background transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50">
                Change Image
              </label>
            </div>
            <FormField
            ...
            <div className="grid grid-cols-2 gap-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input className="w-full bg-transparent p-2" {...field} placeholder={"https://"} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}></FormField>
            ...
            <DialogFooter>
              {pending ? <Loader2 className="animate-spin" /> : <Button type="submit">Save changes</Button>}
            </DialogFooter>
