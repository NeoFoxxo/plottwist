...
export default function ContinueStory({ scenarioData }: CONTINUE_STORY_TYPES) {
  ...
  const [scenario, setScenario] = useState<StoryReturnTypes | null>(scenarioData);
  const [prompt, setPrompt] = useState(scenarioData.prompt ? scenarioData.prompt : "");
  ...
  const isLoading = regeneratePromptRequest.isPending || continueRequest.isPending || finishRequest.isPending;
  ...
  return (
    <section className="flex max-w-[800px] flex-col flex-wrap items-start justify-center gap-4 p-4">
      ...
      <article className="flex flex-col gap-2">
        ...
        <TextGenerateEffect
          className="text-[1.05rem] font-normal"
          words={scenarioData.story ? scenarioData.story : "Loading..."}
        />
      </article>
