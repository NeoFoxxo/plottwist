...
const createSchema = z.object({
  prompt: z.string().min(15, "Minimum characters should be 15!").max(60, "Maximum characters should be 60!"),
});

export default function CreatePrompt({ existingPrompt }: { existingPrompt: string }) {
  ...
  return (
    <>
      {isFormOpen && (
        <Form {...form}>
          <form
            ...
            className="w-full space-y-6 p-4 text-center sm:py-4 md:p-0">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      className="mx-auto w-auto resize-none text-center font-mono sm:w-[35rem]"
                      minLength={15}
                      maxLength={60}
            ...
            <Button variant="default" className="mx-auto w-fit" type="submit" disabled={isDisabled}>
      ...
      {scenario && (
        <section className="flex max-w-[800px] flex-col flex-wrap items-start justify-center gap-4 p-4">
          <h1 className="pb-10 text-4xl font-bold">{scenario.title}</h1>
          <h4 className="text-[1.15rem]">
            <b>Prompt:</b> {scenario.prompt}
          </h4>
          <article className="flex flex-col gap-2">
            <h4 className="text-[1.05rem] font-semibold">Your story:</h4>
            {storyParts.map((currentPart, index) => (
              <TextGenerateEffect key={index} className="text-[1.05rem] font-normal" words={currentPart} />
            ))}
          </article>

          {storyPartCount === 1 ? (
            <Button onClick={regenerate} className="flex items-center justify-center gap-2 font-semibold">
              {pending ? <Loader2 className="animate-spin" /> : <Bot />}
              Regenerate
            </Button>
          ) : (
                ...
                {scenario!!.choices!!.map((choice, index) => {
                  return (
                    <div
                      ...
                      className="w-fit cursor-pointer rounded-md bg-neutral-800 px-4 py-2 hover:bg-neutral-900">
                      {choice}
                    </div>
                  );
                })}
